...
public class BasicQuizActivity extends AppCompatActivity {
    ...
    private String[] questions = {
            ...
            "Which sorting algorithm selects the smallest element and swaps it with the first element?",
    ...
    private String[][] options = {
            ...
            {"O(n log n)","O(n²)","O(n)","O(n + k)"},
            {"Merge Sort","Bubble Sort","Selection Sort","Counting Sort"}
    };

    private int[] correctAnswers = {2, 2, 0, 1, 0};

    private String[] explanations = {
            "Bubble Sort compares adjacent elements and swaps them if needed, making it inefficient for large datasets.",
            "When the input is already sorted, Insertion Sort runs in O(n) time since each element is compared only once.",
            "Selection Sort finds the smallest element in the array and swaps it with the first unsorted element, repeating the process.",
            "Quick Sort has a worst-case complexity of O(n²) when the pivot selection is poor, such as always picking the smallest or largest element.",
            "Merge Sort divides the array into halves, recursively sorts them, and then merges them back."
    };
